...
#ifndef IREEN_CAPABILTY_H
#define IREEN_CAPABILTY_H
...
struct IREEN_EXPORT Capability: public QUuid
{
public:
	enum { Size = 16 };
	Capability();
...
class IREEN_EXPORT Capabilities: public QList<Capability>
{
public:
	enum { UpToFirstZero = 0xff };
	Capabilities();
	Capabilities(const QList<Capability> &other);
...
class IREEN_EXPORT StandartCapability : public Capability
{
public:
...
IREEN_EXPORT QDebug operator<<(QDebug debug, Capability &capability);

} // namespace Ireen

inline uint qHash(const Ireen::Capability &capability)
{
	return capability.hash();
}

#endif // IREEN_CAPABILTY_H
