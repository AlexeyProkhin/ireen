...
void TlvBasedMetaRequestPrivate::addString(quint16 id, MetaFieldEnum key, DataUnit &data, bool test) const
{
	if (!test || values.contains(key)) {
		DataUnit tlv;
		addString(values.value(key).toString(), tlv);
...
{
	QByteArray d = metaInfo->client()->asciiCodec()->fromUnicode(str);
...
template <typename T>
void TlvBasedMetaRequestPrivate::addField(quint16 id, MetaFieldEnum key, DataUnit &data, bool test) const
{
	if (!test || values.contains(key)) {
		DataUnit tlv;
		tlv.append(qVariantValue<T>(values.value(key)), LittleEndian);
		data.appendTLV(id, tlv, LittleEndian);
	}
}

template <typename T>
void TlvBasedMetaRequestPrivate::addCategoryId(quint16 id, MetaFieldEnum key, DataUnit &data, const FieldNamesList &list) const
{
	if (values.contains(key)) {
		QStringList d = values.value(key).toStringList();
		if (!d.isEmpty()) {
...
void TlvBasedMetaRequestPrivate::addCategory(quint16 id, MetaFieldEnum key, DataUnit &data, const FieldNamesList &list) const
{
	if (values.contains(key)) {
		QVariant val = values.value(key);
...
TlvBasedMetaRequest::TlvBasedMetaRequest(MetaInfo *metaInfo, TlvBasedMetaRequestPrivate *d) :
	AbstractMetaRequest(metaInfo, d)
{
}

void TlvBasedMetaRequest::setValue(const MetaFieldKey &key, const QVariant &value)
{
	d_func()->values.insert(key, value);
}
