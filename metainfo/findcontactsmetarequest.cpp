...
FindContactsMetaRequest::FindContactsMetaRequest(MetaInfo *metaInfo, const MetaInfoValuesHash &values) :
	TlvBasedMetaRequest(metaInfo, new FindContactsMetaRequestPrivate)
{
...
bool FindContactsMetaRequest::handleData(quint16 type, const DataUnit &data)
{
	...
	contact.nick = readSString(data, client());
	contact.firstName = readSString(data, client());
	contact.lastName = readSString(data, client());
	contact.email = readSString(data, client());
