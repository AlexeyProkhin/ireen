...
{
	QString str = readSString(data, metaInfo->client());
	if (!str.isEmpty())
...
{
	QHashIterator<MetaFieldKey, QVariant> itr(values);
	while (itr.hasNext()) {
		itr.next();
...
ShortInfoMetaRequest::ShortInfoMetaRequest(MetaInfo *metaInfo, const QString &uin) :
	AbstractMetaRequest(metaInfo, new ShortInfoMetaRequestPrivate)
{
	Q_D(ShortInfoMetaRequest);
	bool ok = false;
	d->uin = uin.isEmpty() ? client()->uin().toUInt(&ok) : uin.toUInt(&ok);
	Q_ASSERT(ok && "Cannot request information about an aim contact");
}
...
QVariant ShortInfoMetaRequest::value(MetaFieldKey key, const QVariant &def) const
{
	return d_func()->values.value(key, def);
}
...
void FullInfoMetaRequestPrivate::readCategories(MetaFieldEnum value, const DataUnit &data, const FieldNamesList &list)
{
	CategoryList result;
	...
	for (int i = 0; i < count; ++i) {
		...
		category.keyword = readSString(data, metaInfo->client());
...
void FullInfoMetaRequestPrivate::handleEmails(const DataUnit &data)
{
	...
	for (int i = 0; i < count; ++i) {
		...
		Q_UNUSED(isPublish);
		QString email = readSString(data, metaInfo->client());
		if (!email.isEmpty())
			emails << email;
	}
	if (!emails.isEmpty())
...
FullInfoMetaRequest::FullInfoMetaRequest(MetaInfo *metaInfo, const QString &uin) :
	ShortInfoMetaRequest(metaInfo, uin)
{
}
...
bool FullInfoMetaRequest::handleData(quint16 type, const DataUnit &data)
{
	...
	case (StateNotes):
		...
		break;
	case (StateInterests):
		d->readCategories(Interests, data, interests());
		break;
	case (StateAffilations): {
		d->readCategories(Pasts, data, pasts());
		d->readCategories(Affilations, data, affilations());
		break;
	}
	default:
